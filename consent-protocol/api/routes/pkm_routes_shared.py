...
async def complete_upgrade_run(
    ...
    try:
        ...
        logger.warning("PKM upgrade complete_run conflict run_id=%s: %s", run_id, exc)
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Upgrade run conflict") from exc
    if payload is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Upgrade run not found")
    return _build_upgrade_status_response(payload)
