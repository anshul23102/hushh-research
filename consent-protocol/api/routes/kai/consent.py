...
# Privileged scopes that may not be self-granted via this endpoint
_SCOPE_BLOCKLIST = {"vault.owner", "agent.nav.revoke", "pkm.write"}


def _assert_scope_allowed(scope_str: str) -> None:
    """Raise HTTPException if scope is in the blocklist."""
    if scope_str in _SCOPE_BLOCKLIST:
        raise HTTPException(
            status_code=403,
            detail=f"Scope {scope_str} is not available through this endpoint.",
        )
